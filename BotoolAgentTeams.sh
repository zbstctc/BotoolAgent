#!/bin/bash
# Botool Agent Teams - 交互式 Agent Teams 模式 (tmux launcher)
# 用法: ./BotoolAgentTeams.sh [--project-dir <path>]
#
...
# 写入初始 .agent-status
# ============================================================================
update_status "starting" "Agent Teams 模式启动中"
...
start_session() {
  ...
  echo ">>> Agent Teams session started: $SESSION_NAME"
  echo ""
...
for CURRENT_ROUND in $(seq 1 $MAX_ROUNDS); do
  ...
  echo ""
  echo "==============================================================="
  echo "  Agent Teams - 轮次 $CURRENT_ROUND/$MAX_ROUNDS (已完成 $completed/$total)"
  echo "==============================================================="

  # 3. 更新状态
  update_status "running" "Agent Teams 轮次 $CURRENT_ROUND/$MAX_ROUNDS"
