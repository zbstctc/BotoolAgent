...
Claude Code 的 AskUserQuestion 有固定 UI 元素（Type something / Chat about this），因此方案卡的 options 固定为 **3 个选项**（其中 Type something 是整卡级“其他（自行输入）”逃生口）：

```
UI 显示效果:
  1. 全部接受 (推荐)                  ← 80% 用户直接选
  2. 方向不对，重新生成                ← 逃生口，AI 重出方案卡
  3. ↓ 请在下方 Type something 输入修改  ← 引导到整卡级 Other（自行输入）
...
| 文件 | 生产方 | 消费方 | 生命周期 |
|------|-------|-------|---------|
| `tasks/<id>/qa-journal.md` | PyramidPRD 主对话 | PyramidPRD 各层 + PRD Subagent + PRD2JSON(A2) | 问答完成后保留（调试用）|
...
| BR-037 | **方案卡每题 2-4 选项 + 整卡级逃生口** | 每题 2-4 个预设选项；卡片级通过 Type something 提供“其他（自行输入）”；> 4 选项 → 拆分问题 | DT-021 |
...
- [ ] 方案卡“全部接受”选择率 ≥ 60%（统计口径：最近 30 次方案卡交互，来源 qa-journal.md）
...
| **review-save 覆盖 prd.md** | ✅ **保留**（仅 A1 auto-chain 使用 review-save 覆盖 `tasks/<id>/prd.md`；Standalone 走“备份 `{stem}_original.md` + 覆盖目标文件”流程，不走 review-save API） |
...
| ~~多项目选择（AskUserQuestion）~~ | ⛔ **已删除**（独立项目审查模式删除后不再需要） |
...
| 8 | 方案卡选项数量 | **每题 2-4 个 + 整卡级“其他（自行输入）”** | 单题 > 4 选项应拆分；自由输入通过卡片级 Type something 处理 |
