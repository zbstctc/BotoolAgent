...
cp -r skills/botool_dev_prd ~/.claude/skills/
cp -r skills/botool_dev_prd_json ~/.claude/skills/
```
...
```
/botool_dev_prd or "create a prd for [your feature description]"
```
...
```
/botool_dev_prd_json or "convert tasks/prd-[feature-name].md to prd.json"
```
...
| `skills/botool_dev_prd/` | Skill for generating PRDs |
| `skills/botool_dev_prd_json/` | Skill for converting PRDs to JSON |
...
Frontend stories should include "Verify in browser" in acceptance criteria. If Playwright MCP is configured, the agent will navigate to the page, interact with the UI, and confirm changes work.
