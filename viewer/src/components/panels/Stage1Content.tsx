...
export interface Stage1ContentProps {
  reqId: string;
}

export function Stage1Content({ reqId: reqIdProp }: Stage1ContentProps) {
  const router = useRouter();
  const searchParams = useSearchParams();

  // Requirement context - use reqId from prop
  const { requirements, updateRequirement } = useRequirement();
  const reqId = reqIdProp || undefined;
