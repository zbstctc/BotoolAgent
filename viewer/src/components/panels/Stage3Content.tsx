...
export default function Stage3Content({ reqId }: Stage3ContentProps) {
  const router = useRouter();

  // Requirement context - resolve `req` param
  const { requirements } = useRequirement();
  const activeRequirement = requirements.find(r => r.id === reqId);

  // projectId: req.id as fallback (no more searchParams for projectId)
  const projectId = activeRequirement?.id ?? reqId;
