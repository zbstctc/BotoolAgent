...
每个 Step 只聚焦 **一个维度**，用户逐步确认，确保每一层信息都被精准审视。

| 步骤 | 快速修复 | 功能开发 | 完整规划 |
|------|---------|---------|---------|
| Step 1: 项目概述 | 跳过 | 展示 | 展示 |
| Step 2: 核心架构 | 跳过 | 展示 | 展示 |
| Step 3: 用户角色 | 跳过 | 展示 | 展示 |
| Step 4: 核心工作流 | 跳过 | 展示 | 展示 |
| Step 5: 数据模型 | 跳过 | 跳过 | 展示 |
| Step 6: 关键 UI | 跳过 | 跳过 | 展示（仅有前端时） |
| Step 7: 业务规则 | 跳过 | 跳过 | 展示 |
| Step 8: 开发计划 + 风险 + 总确认 | 跳过 | 展示 | 展示 |

**快速修复模式：** 跳过 L5，直接生成。

**设计原则：** 每个 Step 的 ASCII 可视化内容必须聚焦单一维度，禁止在一个 Step 中混合多个维度。这确保用户能逐个审视、逐个确认，避免信息过载。

---

#### Step 1: 项目概述确认

展示项目的基本定位和技术选型，让用户确认「我们要做什么」：

```
【L5: 确认门控 — Step 1/8 项目概述】
  ...
  "questions": [
    {
      ...
      "header": "项目概述",
      "options": [
        { "label": "准确，继续", "description": "项目定位、目标和技术栈描述正确" },
...
展示系统的核心实体和它们之间的关系，让用户确认「系统的骨架长什么样」：

```
【L5: 确认门控 — Step 2/8 核心架构】
  ...
  "questions": [
    {
      ...
      "header": "核心架构",
      "options": [
        { "label": "准确，继续", "description": "实体关系和职责描述正确" },
        { "label": "需要修改", "description": "有不准确的地方，我会说明" }
      ],
      "multiSelect": false
    }
...
如果用户选择「需要修改」，根据用户反馈调整后重新展示 Step 2。

---

#### Step 3: 用户角色确认

展示系统中的用户角色、权限和访问路径，让用户确认「谁能做什么」：

```
【L5: 确认门控 — Step 3/8 用户角色】
  ...
  "questions": [
    {
      ...
      "header": "用户角色",
      "options": [
        { "label": "准确，继续", "description": "角色划分和权限描述正确" },
        { "label": "需要修改", "description": "有不准确的地方，我会说明" }
      ],
      "multiSelect": false
    }
...
如果用户选择「需要修改」，根据用户反馈调整后重新展示 Step 3。

---

#### Step 4: 核心工作流确认

展示开发阶段划分、优先级和依赖关系，让用户确认「按什么顺序做」：

```
【L5: 确认门控 — Step 4/8 核心工作流】
  ...
  "questions": [
    {
      ...
      "header": "核心工作流",
      "options": [
        { "label": "准确，继续", "description": "阶段划分、优先级和依赖描述正确" },
        { "label": "需要修改", "description": "有不准确的地方，我会说明" }
      ],
      "multiSelect": false
    }
...
如果用户选择「需要修改」，根据用户反馈调整后重新展示 Step 4。

---

#### Step 5: 数据模型确认（仅完整规划）

展示数据模型的字段设计和表间关系，让用户确认「数据怎么存」：

```
【L5: 确认门控 — Step 5/8 数据模型】
  ...
  "questions": [
    {
      ...
      "header": "数据模型",
      "options": [
        { "label": "准确，继续", "description": "数据表、字段和关系描述正确" },
        { "label": "需要修改", "description": "有不准确的地方，我会说明" }
      ],
      "multiSelect": false
    }
...
如果用户选择「需要修改」，根据用户反馈调整后重新展示 Step 5。

---

#### Step 6: 关键 UI 确认（仅完整规划 + 有前端）

展示关键页面的 ASCII 线框布局，让用户确认「界面长什么样」。**每个关键页面/弹窗单独展示**，如果页面较多则分多轮确认：

```
【L5: 确认门控 — Step 6/8 关键 UI】
  ...
  "questions": [
    {
      ...
      "header": "关键 UI",
      "options": [
        { "label": "准确，继续", "description": "页面布局和交互流程描述正确" },
        { "label": "需要修改", "description": "有不准确的地方，我会说明" }
      ],
      "multiSelect": false
    }
...
如果用户选择「需要修改」，根据用户反馈调整后重新展示 Step 6。

---

#### Step 7: 业务规则确认（仅完整规划）

展示核心业务规则和决策树，让用户确认「业务逻辑怎么跑」：

```
【L5: 确认门控 — Step 7/8 业务规则】
  ...
  "questions": [
    {
      ...
      "header": "业务规则",
      "options": [
        { "label": "准确，继续", "description": "业务规则和决策逻辑描述正确" },
        { "label": "需要修改", "description": "有不准确的地方，我会说明" }
      ],
      "multiSelect": false
    }
...
如果用户选择「需要修改」，根据用户反馈调整后重新展示 Step 7。

---

#### Step 8: 开发计划 + 风险 + 总确认

展示开发计划、风险评估和复杂度估计，作为最终总确认。**此步骤不重复前 7 步已确认的内容**，只聚焦执行层面：

```
【L5: 确认门控 — Step 8/8 总确认】
...
```
Phase T6 输出 → 喂入现有 Phase 6 (L5 Confirmation Gate)
  Step 1: 架构确认（从用户 PRD 提取的架构）
  Step 2: 数据模型确认（从用户 PRD 提取的 Schema）
  Step 3: UI 确认（提取 + 补充的 UI 设计）
  Step 4: 业务规则确认（提取 + 补充的规则）
  Step 5: 开发计划 + 总确认（展示拆解后的 DT 列表）
```

**按模式裁剪 L5 步骤：** Transform 模式使用与「完整规划」相同的 L5 步骤配置（Step 1-5 全部展示），因为用户的原始 PRD 通常涵盖较多维度。
...
|------|-------------|------------|
| 问答结构 | 自由对话（一问一答） | 固定 5 层（含确认门控） |
| 问题数量 | 灵活 | 根据复杂度动态调整（3-12 个/层） |
| 复杂度感知 | 按文件数粗略分级 | 自动评估，简单需求快速完成 |
| 确认门控 | 无 | L5 多步 ASCII 确认，可返回修改 |
| PRD 模板 | 同模板（按复杂度裁剪） | 同模板（按复杂度裁剪） |
| ASCII 图 | 复杂功能时有 | 完整规划模式时有 |
| 进度可视化 | 无 | metadata 中的 level |
| 适用场景 | CLI 对话（灵活） | Viewer Stage 1（结构化） |
