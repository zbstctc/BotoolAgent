...
- 禁止将"无法匹配"的章节静默跳过
- 必须读取该章节前 30 行，基于**内容语义**强制分配到最合适的目标维度
- 在工作笔记中标注：`⚠️ [源章节] 「无关键词匹配」→ 语义判断 → §Y（理由: 内容含...）`
...
**跳过条件：** 源 PRD < 5000 行（非大文件模式）可跳过此阶段（步骤 A-D 全部跳过）。
...
| 2000-5000 行 | 最低 源 PRD 的 60% | 去冗余后核心内容全部保留 |
| > 5000 行 | 最低 2000 行（不达标必须补充）| 大文件去冗余后仍应保持高信息密度 |

#### 强制自检（大文件模式，生成完毕后必须执行）

生成 PRD 完成后，执行以下命令自检：

1. `grep -c "CREATE TABLE" $TASKS_DIR/<projectId>/prd.md`
   对比 T1「必须保留表清单」，确认 CREATE TABLE 数量 ≥ 源 PRD 的 80%

2. `wc -l $TASKS_DIR/<projectId>/prd.md`
   确认总行数满足最低要求（源 PRD > 5000 行时 ≥ 2000 行）

生成失败条件（满足任一 → 必须补充内容后重新确认，不可跳过）：
- ❌ 生成 PRD 行数 < 2000 行（源 PRD > 5000 行时）
- ❌ CREATE TABLE 数量 < 源 PRD 表数量的 80%
- ❌ T2.5「关键字段清单」中的字段在生成 PRD 中完全找不到

满足上述失败条件时，不进入 Phase T5 DT 分解，而是补充生成缺失内容后重试。
