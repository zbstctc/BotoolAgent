...
**核心原则：** 用户的 PRD 只是输入材料。BotoolAgent 始终需要自己格式的 PRD，包含正确粒度的 DT 分解。
...
每个维度桶：
  - 计算该桶所有章节的总行数
  - 如果 ≤ 500 行 → 完整读取
  - 如果 > 500 行 → 读前 300 行 + 后 200 行（跳过中间重复内容）
  - 提取关键信息到结构化摘要
```

**安全限制：**
- 最多 10 次 Read 调用（避免上下文爆炸）
- 优先读取 § 7（开发计划）和 § 4（数据设计），这两个对 DT 分解最关键
- 跳过明显无关的章节（更新日志、会议纪要等）
...
| 覆盖度 | 图示 | 含义 | 后续动作 |
|--------|------|------|----------|
| FULL | ████ | 内容充分，可直接提取 | 自动映射，不提问 |
| HIGH | ███░ | 基本覆盖，有小模糊点 | 1-2 个确认性问题 |
| PARTIAL | ██░░ | 部分内容，关键部分缺失 | 3-5 个针对性问题 |
| SPARSE | █░░░ | 几乎没有内容 | 复用现有 L2/L3 问题模板 |
...
用户确认后，进入现有的 Phase 7（PRD 生成）生成标准 BotoolAgent PRD。
