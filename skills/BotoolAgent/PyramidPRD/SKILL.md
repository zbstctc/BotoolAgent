...
**目标：** 在覆盖度评分前，验证"已读内容"与"T1 预提取基准"一致，防止关键表/字段无声消失。
...
4. 向用户展示校验结果（纯文本输出，不使用 AskUserQuestion）：

   若完整：
   > "✅ T2.5 校验通过：已完整读取 X 个表（共 Y 个），关键字段均已捕获，进入覆盖度分析。"

   若有缺失：
   > "⚠️ T2.5 发现缺失：以下表/字段未被记录：
   >    - present_collaborators（整表缺失）→ 正在补充读取...
   >    - categories.parent_id（字段未标注）→ 正在补充标注..."
   > 补充完成后，重新执行步骤 1-3 直至全部通过。

**跳过条件：** 源 PRD < 5000 行（非大文件模式）可跳过此阶段。
